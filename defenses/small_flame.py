...
# def flame_module(fc_model_list, global_fc_model, device):
#     fc_avg = copy.deepcopy(global_fc_model)
#     cos = []
#     cos_ = []
#
#     for fc_model_out in fc_model_list:
#         x1 = vectorize_net(fc_model_out) - vectorize_net(global_fc_model)
#         for fc_model_in in fc_model_list:
#             x2 = vectorize_net(fc_model_in) - vectorize_net(global_fc_model)
...
#
#     e = []  # 每个模型和全局模型的欧氏距离
#     for i in range(len(fc_model_list)):
#         e.append(
#             torch.sqrt(
#                 torch.sum(
#                     (vectorize_net(fc_avg) -
#                      vectorize_net(fc_model_list[i])) ** 2)))
#
...
#         wa.append(fc_avg[p])
#
#     for p_index, p in enumerate(fc_model_list[0]):
#         # initial
#         params_aggregator = torch.zeros(fc_model_list[0][p].size()).to(device)
#
#         for i in range(len(out)):
#             net = fc_model_list[out[i]]
...
def small_flame(model_list, global_model, device):
    ...
    new_global_model = replace_fc_layers(global_model, fc_avg)
